...
| 商品画像表示 | LLM が提供する画像URLで商品サムネイルを表示（フォールバック付き） |
| おすすめ理由 (AI Reason) | LLM が各商品の推薦理由を構造化して表示 |
| Pros / Cons | 各商品の良い点・注意点をリスト表示 |
| 商品比較テーブル | 複数商品のスペックを横並び比較テーブルで表示 |
...
```
Sony WH-1000XM5 と Bose QC Ultra を比較して
```
```
人気のワイヤレスヘッドホンを3つ比較して
```
```
HHKB と RealForce どっちがいい？
```
...
| `compare_products` | 複数商品のスペック比較テーブルを表示 | ChatGPT (LLM) |
  ...
  "products": [
    {
      ...
      "recommendReason": "NC性能が業界最高クラス。30時間バッテリーで長時間使用OK",
...
### compare_products の入力スキーマ

LLM が比較対象の商品をスペック付きで渡します：
...
Widget 上でスペック比較テーブルとして横並びに表示されます。
