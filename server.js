...
function createAdServer() {
  ...
  registerAppTool(server, "search_products", {
    ...
    description:
      ...
      "asin は設定しないでください。imageUrl も省略してください。" +
      "商品リンクと画像はサーバーが商品名とブランド名をもとに自動生成します。" +
    ...
    // Enrich products:
    // - amazonUrl: 「ブランド名 商品名」でAmazon検索URL → 必ず存在するページ
    // - imageUrl: サーバー側で商品名ベースのUnsplash画像を使用（ハルシネーションASIN画像は使わない）
    const enrichedProducts = products.map((p, i) => ({
      ...p,
      id: `prod-${i}`,
      amazonUrl: buildAmazonSearchUrl(`${p.brand} ${p.title}`),
      imageUrl: null,  // 番号フォールバック表示（画像ハルシネーション回避）
  ...
  registerAppTool(server, "compare_products", {
    ...
    description:
      ...
      "asin と imageUrl は設定しないでください。商品リンクはサーバーが自動生成します。" +
    ...
    const enrichedProducts = products.map((p, i) => ({
      ...
      id: `cmp-${i}`,
      amazonUrl: buildAmazonSearchUrl(`${p.brand} ${p.title}`),
      imageUrl: null,  // 番号フォールバック（ハルシネーション画像回避）
    }));
